...
CREATE TABLE IF NOT EXISTS `lto_news`.`usuario` (
  ...
  `rol_id` VARCHAR(32) NOT NULL,
  PRIMARY KEY (`usu_id`),
  INDEX `USUROL_FK_idx` (`rol_id` ASC),
  CONSTRAINT `USUROLFK`
    FOREIGN KEY (`rol_id`)
    REFERENCES `lto_news`.`rol` (`rol_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB
DEFAULT CHARACTER SET = latin1;
...
CREATE TABLE IF NOT EXISTS `lto_news`.`comentario` (
  ...
  `com_noticia` VARCHAR(32) NOT NULL,
  `com_usuario` VARCHAR(32) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `lto_news`.`noticia_imagen` (
  `nim_id` VARCHAR(32) NOT NULL,
  `img_id` VARCHAR(32) NOT NULL,
  `not_id` VARCHAR(32) NOT NULL,
  INDEX `NIMNOTFK_idx` (`not_id` ASC),
  INDEX `NIMIMGFK_idx` (`img_id` ASC),
  PRIMARY KEY (`nim_id`),
  CONSTRAINT `NIMIMGFK`
    FOREIGN KEY (`img_id`)
